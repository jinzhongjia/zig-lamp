...
function M.get_lamp()
    ...
    if _zig_lamp == true then return nil end
    
    -- Return cached instance
    if _zig_lamp then return _zig_lamp end
...
function M.lazy_load()
    if _zig_lamp ~= true then return end
...
function M.get_build_zon_info(file_path)
    ...
    if not path:new(file_path):exists() then return nil end
